...
def add_points(input_state, action_vec, board_type, construction):
    ...
    tic = time.time()
    if point_added:
        terminal = count_isosceles_triangles(ones_indices, action_index, n, board_type, construction) > 0

    count_time += time.time() - tic
    return cur_state, action_taken, terminal

# Defining a function to generate a new session
def generate_session(agent, n_sessions, n, verbose = 1, board_type = "euc", construction = "rowwise"):
    global copy_time
    global add_time
    global count_time
    ...
    actions = torch.zeros([n_sessions, n*n, n*n], dtype = torch.int)
    ...
    for i in range(n_sessions):
        ...
        while not terminal:
            ...
            tic = time.time()
            new_state, action, terminal = add_points(cur_state, output, board_type, construction)
...
# Defining a function to generate a new session
def generate_session_thresholding(agent, n_sessions, n, acceptence_threshold, explore_rate, verbose = 1):
    ...
    #### Variables for States, Actions, and Rewards ####
    ...
    for i in range(n_sessions):
        ...
        ith_game = torch.zeros((n*n, n*n), dtype=torch.float)
        ith_game.to(device)
        ith_game_actions = torch.zeros([n*n, n*n])
        score = 0

        while not terminal:
            step+=1
            cur_state = ith_game[step-1, :]
            ...
            ith_game_actions[step-1, :] = action
            scoreUpdate_time += time.time() - tic

            tic = time.time()
            ith_game[step, :] = new_state
            recordsess_time += time.time() - tic

        if score > acceptence_threshold or np.random.rand() < explore_rate:
            states.append(ith_game)
            actions.append(ith_game_actions)
...
def train(board_size, write_all, write_best, filename, slow, old_net = None, board_type = "euc", construction = "rowwise", freeze = 0):

    n = board_size
    acceptence_threshold = np.inf #0.4*n
    # explore_rate = 0.6
    ...
    # If a previous network is provided, copy the weights from the previous network to the new network
    if old_net is not None:
        copy_weights(old_net, net)
    ...
    for i in range(15000):
        ...
        #print("ACCEPTENCE THRESHOLD: " + str(acceptence_threshold))

        tic = time.time()
        states_batch, actions_batch, rewards_batch = generate_session(net, n_sessions, n, verbose, board_type, construction)
        #states_batch, actions_batch, rewards_batch = generate_session_thresholding(net, n_sessions, n, acceptence_threshold, explore_rate, verbose)
        sessgen_time = time.time() - tic

        states_batch = states_batch.to(dtype=torch.int)

        #if construction == "sym", scale the rewards by 4
        if construction == "sym":
            rewards_batch = rewards_batch * 4
        ...
        # Copy the weights from the old network to the new network
        if old_net is not None and freeze == 1:
            copy_weights(old_net, net)
